function val=gamma4(N,FA,Q1,Q2,Q3,Q4)
...
if norm(Q1+Q2+Q3+Q4)>=MIN
    ...
    s2ainv=s2inverse(N,FA,norm(Q1));
    s2binv=s2inverse(N,FA,norm(Q2));
    s2cinv=s2inverse(N,FA,norm(Q3));
    s2dinv=s2inverse(N,FA,norm(Q4));
    ...
    for I = 1:length(M)
        val = val + g4(M(I,1),M(I,2),M(I,3),M(I,4))*...
                (s2ainv(M(I,1),1)-s2ainv(M(I,1),2))*...
                (s2binv(M(I,2),1)-s2binv(M(I,2),2))*...
                (s2cinv(M(I,3),1)-s2cinv(M(I,3),2))*...
                (s2dinv(M(I,4),1)-s2dinv(M(I,4),2));
    end
end
