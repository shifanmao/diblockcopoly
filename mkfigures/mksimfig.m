...
for EPS = [1.0]
    ...
    [KRPA1,SRPA1,CHISIM1,SINVSIM1] = plotsimfig(EPS,Gtimes2,RUNNUM,simparams,LBOX,DEL);
    
    RUNNUM = 3;LBOX = 16;
    if (RUNNUM == 2 || RUNNUM == 3)
        C = sqrt(1525.9);
    end
    [KRPA2,SRPA2,CHISIM2,SINVSIM2] = plotsimfig(EPS,Gtimes2,RUNNUM,simparams,LBOX,DEL);
    
    PLOTDENSITY = 1;PLOTRG = 0;NCHI = 50;
...
%     figure;plot(KRPA1,SRPA1);title(sprintf('N=%.2f, C^2=%.2f',N,C^2))
%     figure;plot(KRPA2,SRPA2);title(sprintf('N=%.2f, C^2=%.2f',N,C^2))
end

cd mkfigures/
end

function [KRPA,SRPA,CHISIM,SINVSIM] = plotsimfig(EPS,Gtimes2,RUNNUM,simparams,LBOX,DEL)
...
if LBOX == 20
    plot(CHISIM*PREF,SINVSIM*Gtimes2,'b^','markersize',8)
elseif LBOX == 16
    plot(CHISIM*PREF,SINVSIM*Gtimes2,'b<','markersize',8)
end
